...
public final class ErrorNotificationService {
  ...
  public ErrorNotificationService(EmailService emailService, String from, String to) {
    this.emailService = Objects.requireNonNull(emailService, "emailService");
    this.from = Objects.requireNonNull(from, "from");
    this.to = Objects.requireNonNull(to, "to");
  }

  public void notifyError(Throwable ex, String contexto, Properties props) {
    notifyErrorAndReturnResult(ex, contexto, props);
  }

  public boolean notifyErrorAndReturnResult(Throwable ex, String contexto, Properties props) {
    try {
      EmailData errorEmail = ErrorEmailBuilder.build(ex, contexto, from, to);
